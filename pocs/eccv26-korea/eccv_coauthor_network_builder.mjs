...
<section id="eccv-coauthor-network" aria-labelledby="eccv-network-title">
  <style>
    ...
    #eccv-coauthor-network .selection-pulse-edge { animation: selection-edge-pulse 280ms ease-out var(--selection-pulse-delay, 0ms) 1 both; pointer-events: none; stroke: var(--selection-pulse-color); stroke-linecap: round; stroke-width: 2.2px; }
    #eccv-coauthor-network .selection-pulse-node { animation: selection-node-pulse 300ms ease-out var(--selection-pulse-delay, 0ms) 1 both; fill: none; pointer-events: none; stroke: var(--selection-pulse-color); }
    @keyframes selection-edge-pulse { 0% { opacity: 0; stroke-width: .5px; } 40% { opacity: .9; stroke-width: 2.8px; } 100% { opacity: 0; stroke-width: .5px; } }
    @keyframes selection-node-pulse { 0% { opacity: 0; stroke-width: .4px; } 35% { opacity: .9; stroke-width: 2.7px; } 100% { opacity: 0; stroke-width: .4px; } }
    @media (prefers-reduced-motion: reduce) { #eccv-coauthor-network .selection-pulse-edge, #eccv-coauthor-network .selection-pulse-node { animation: none; display: none; } }
  ...
  (() => {
    ...
    const neighbourEdges = new Map(nodes.map((d) => [d.id, []]));
    links.forEach((d) => {
      neighbourEdges.get(d.source).push({ edge: d, nodeId: d.target });
      neighbourEdges.get(d.target).push({ edge: d, nodeId: d.source });
    });
    ...
    const selectionPulseLayer = scene.append('g').attr('aria-hidden', 'true').attr('class', 'selection-pulse-layer');
    const selectionPulseEdges = selectionPulseLayer.append('g').attr('class', 'selection-pulse-edges');
    const selectionPulseNodes = selectionPulseLayer.append('g').attr('class', 'selection-pulse-nodes');
    ...
    const connectedComponentTraversal = (startId) => {
      ...
      while (pending.length) {
        ...
        for (const { edge, nodeId } of neighbourEdges.get(currentId) ?? []) {
          if (distances.has(nodeId)) continue;
          const distance = currentDistance + 1;
          distances.set(nodeId, distance);
          treeEdges.push({ distance, edge, sourceId: currentId, targetId: nodeId });
          pending.push(nodeId);
        }
      }
      return { distances, treeEdges };
    };
    const connectedComponentIds = (startId) => new Set(connectedComponentTraversal(startId).distances.keys());
    const clearSelectionPulse = () => {
      selectionPulseEdges.selectAll('line').interrupt().remove();
      selectionPulseNodes.selectAll('circle').interrupt().remove();
    };
    const startSelectionPulse = (startId) => {
      clearSelectionPulse();
      const selectedNode = nodeById.get(startId);
      if (!selectedNode) return;
      const { distances, treeEdges } = connectedComponentTraversal(startId);
      const pulseColor = color(selectedNode.institution);
      selectionPulseEdges.selectAll('line')
        .data(treeEdges, (d) => d.sourceId + '→' + d.targetId)
        .join('line')
        .attr('class', 'selection-pulse-edge')
        .style('--selection-pulse-color', pulseColor)
        .style('--selection-pulse-delay', (d) => (d.distance * 180) + 'ms');
      selectionPulseNodes.selectAll('circle')
        .data([...distances.entries()].filter(([id]) => id !== startId), ([id]) => id)
        .join('circle')
        .attr('class', 'selection-pulse-node')
        .attr('r', ([id]) => radius(nodeById.get(id)) + 4)
        .style('--selection-pulse-color', pulseColor)
    ...
    const renderLayout = () => {
      ...
      selectionPulseEdges.selectAll('line').attr('x1', (d) => d.edge.source.x).attr('y1', (d) => d.edge.source.y).attr('x2', (d) => d.edge.target.x).attr('y2', (d) => d.edge.target.y);
      selectionPulseNodes.selectAll('circle').attr('cx', ([id]) => nodeById.get(id).x).attr('cy', ([id]) => nodeById.get(id).y);
